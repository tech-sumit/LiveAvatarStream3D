...
End-to-end build sequence. Each phase has an explicit exit criterion. This was an internal tool at the time, so auth/consent/hardening were deferred (see bottom).
...
## Deferred (not in scope during the internal-tool era)
Auth/login, multi-tenant isolation, consent capture + watermarking + moderation, billing/quotas, dashboards/alerting, public open-source packaging + license matrix — deferred at the time, not permanently out of scope.
