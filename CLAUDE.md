...
The editor's `.env.development` must point `VITE_API_URL` at the **deployed Worker** (`https://<your-worker>.workers.dev/api`), not local `wrangler dev` — local dev has isolated D1/R2 so cloned voices won't be found on the GPU.
