...
same calls the web app makes — to prove the offline MVP end to end:
  ...
  upload demo_video.mp4 -> build avatar "demo-ref" -> clone voice ->
...
def _default_script() -> dict:
    return {
        ...
        "segments": [
            {
                "seq": 0,
                "text": "Hi, I'm demo-ref. This is an end to end offline render straight from my reference video.",
                "emotion": "warm",
                "gesture": "open_palms",
                "posture": "upright",
                "emphasis": ["demo-ref", "offline"],
                "pause_ms_after": 300,
            },
            {
                "seq": 1,
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Offline 1080p render e2e validation (avatar 'demo-ref').")
    ...
    ap.add_argument("--out", default="/tmp/demo-ref_offline.mp4", help="where to save the finished mp4")
    ...
    # 1) Build avatar "demo-ref" from the reference video.
    ...
    sample_path = _extract_voice_sample(args.video, "/tmp/demo-ref_voice_sample.wav", args.voice_sample_seconds)
